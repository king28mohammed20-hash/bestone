...
from wtforms import StringField, PasswordField, SubmitField, TextAreaField, SelectField, DateField, DecimalField, IntegerField, BooleanField, FileField
...
CLOSED_WEEKDAYS = set(int(x) for x in os.environ.get("CLOSED_WEEKDAYS", "").split(",") if x.strip().isdigit())
...
def time_slots(step_minutes: int = None):
...
def within_business_hours(dt: datetime) -> bool:
    return (OPEN_HOUR <= dt.hour < CLOSE_HOUR) and not (dt.hour == CLOSE_HOUR - 1 and dt.minute > 30)
...
def save_image(file_storage, prefix="img"):
    ...
    with Image.open(abs_path) as im:
        ...
        # Main image: contain, then letterbox on white canvas
...
@app.route("/")
def index():
    services = db.session.scalars(db.select(Service).where(Service.active == True).order_by(Service.name)).all()
    offers = db.session.scalars(db.select(Offer).where(Offer.active == True).order_by(Offer.id.desc())).all()
    contact = ContactInfo.get_single(create_if_missing=True)
    return render_template("index.html", services=services, offers=offers, contact=contact)
...
def book():
    ...
    if form.validate_on_submit():
        ...
        if appt_dt < datetime.now():
            flash("لا يمكن اختيار وقت في الماضي.", "warning"); return render_template("booking_form.html", form=form)
        if not within_business_hours(appt_dt):
            flash("الدوام من 1:00 ظهرًا إلى 10:00 مساءً (آخر موعد يبدأ 9:30).", "warning"); return render_template("booking_form.html", form=form)
        if is_conflicting(form.service_id.data, appt_dt):
            flash("هذا الموعد محجوز بالفعل لهذه الخدمة. اختر وقتًا آخر.", "warning"); return render_template("booking_form.html", form=form)
        b = Booking(user_id=current_user.id, service_id=form.service_id.data, appointment_at=appt_dt, notes=form.notes.data or "")
        db.session.add(b); db.session.commit()
        flash("تم إرسال طلب الحجز. بانتظار موافقة الإدارة.", "success"); return redirect(url_for("my_bookings"))
    return render_template("booking_form.html", form=form)

@app.route("/my-bookings")
@login_required
def my_bookings():
    items = db.session.scalars(db.select(Booking).where(Booking.user_id == current_user.id).order_by(Booking.appointment_at.desc())).all()
...
def admin_home():
    admin_required()
    pending = db.session.scalars(db.select(Booking).where(Booking.status == "pending").order_by(Booking.appointment_at.asc())).all()
    return render_template("admin_home.html", pending=pending)
...
def admin_service_new():
    ...
    form = ServiceForm()
    if form.validate_on_submit():
        s = Service(name=form.name.data.strip(), price=form.price.data, duration_minutes=form.duration_minutes.data, active=True)
        if "image" in request.files and request.files["image"].filename:
            try:
                rel = save_image(request.files["image"], prefix="service"); s.image_path = rel
            except Exception as e:
                flash(str(e), "danger"); return render_template("admin_service_form.html", form=form, is_edit=False)
        db.session.add(s); db.session.commit(); flash("تمت إضافة الخدمة.", "success"); return redirect(url_for("admin_services"))
...
def admin_service_edit(service_id):
    ...
    if form.validate_on_submit():
        s.name = form.name.data.strip(); s.price = form.price.data; s.duration_minutes = form.duration_minutes.data; s.active=True
        if "image" in request.files and request.files["image"].filename:
            try:
                if s.image_path: delete_image(s.image_path)
                rel = save_image(request.files["image"], prefix="service"); s.image_path = rel
            except Exception as e:
                flash(str(e), "danger"); return render_template("admin_service_form.html", form=form, is_edit=True)
        db.session.commit(); flash("تم تعديل الخدمة.", "success"); return redirect(url_for("admin_services"))
...
def admin_service_delete(service_id):
    ...
    s = db.session.get(Service, service_id) or abort(404)
    if s.image_path: delete_image(s.image_path)
    db.session.delete(s); db.session.commit(); flash("تم حذف الخدمة.", "info"); return redirect(url_for("admin_services"))
...
def admin_offer_new():
    admin_required()
    form = OfferForm(); form.service_id.choices = _service_choices()
    if form.validate_on_submit():
        service_id = form.service_id.data or 0
        o = Offer(title=form.title.data.strip(), description=form.description.data or "", price=form.price.data if form.price.data is not None else None, active=True, service_id=(service_id if service_id != 0 else None))
        if "image" in request.files and request.files["image"].filename:
            try:
                rel = save_image(request.files["image"], prefix="offer"); o.image_path = rel
            except Exception as e:
                flash(str(e), "danger"); return render_template("admin_offer_form.html", form=form, is_edit=False)
        db.session.add(o); db.session.commit(); flash("تمت إضافة الإعلان.", "success"); return redirect(url_for("admin_offers"))
...
def admin_offer_edit(offer_id):
    ...
    form = OfferForm(obj=o); form.service_id.choices = _service_choices(); form.service_id.data = o.service_id or 0
    if form.validate_on_submit():
        o.title = form.title.data.strip(); o.description = form.description.data or ""; o.price = form.price.data if form.price.data is not None else None; o.active=True; o.service_id = (form.service_id.data if form.service_id.data != 0 else None)
        if "image" in request.files and request.files["image"].filename:
            try:
                if o.image_path: delete_image(o.image_path)
                rel = save_image(request.files["image"], prefix="offer"); o.image_path = rel
            except Exception as e:
                flash(str(e), "danger"); return render_template("admin_offer_form.html", form=form, is_edit=True)
        db.session.commit(); flash("تم تعديل الإعلان.", "success"); return redirect(url_for("admin_offers"))
...
def admin_offer_delete(offer_id):
    ...
    o = db.session.get(Offer, offer_id) or abort(404)
    if o.image_path: delete_image(o.image_path)
    db.session.delete(o); db.session.commit(); flash("تم حذف الإعلان.", "info"); return redirect(url_for("admin_offers"))
...
def admin_approve(booking_id):
    ...
    if not b: abort(404)
    b.status = "approved"; db.session.commit(); flash("تمت الموافقة على الحجز.", "success"); return redirect(url_for("admin_home"))
...
def admin_cancel(booking_id):
    ...
    b = db.session.get(Booking, booking_id)
    if not b: abort(404)
    b.status = "cancelled"; db.session.commit(); flash("تم إلغاء الحجز.", "info"); return redirect(url_for("admin_home"))
...
def init_db():
    ...
    admin_email = os.environ.get("ADMIN_EMAIL", "admin@example.com").lower(); admin_password = os.environ.get("ADMIN_PASSWORD", "admin123"); admin_name = os.environ.get("ADMIN_NAME", "Administrator")
    admin = User(full_name=admin_name, email=admin_email, is_admin=True); admin.set_password(admin_password); db.session.add(admin)
    services = [Service(name="تظليل", price=35, duration_minutes=60, active=True),
    ...
    demo_offer = Offer(title="عرض تظليل خاص", description="خصم على التظليل لهذا الأسبوع.", price=30, active=True, service_id=None)
    db.session.add(demo_offer); db.session.commit()
